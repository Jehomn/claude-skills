...
name: bb-analysis
description: |
  双轴分析法（Horizontal-Vertical Analysis）深度研究Skill。融合了索绪尔的历时-共时分析、社会科学的纵向-横截面研究设计、商学院案例研究法与竞争战略分析的核心思想。
...
> **方法论溯源**
> 双轴分析法融合了语言学中的历时-共时分析（Saussure）、社会科学中的纵向-横截面研究设计、商学院案例研究法、以及竞争战略分析的核心思想，形成了一套适用于产品/公司/概念/人物的通用研究框架。核心原则不变：纵向追时间深度，横向追同期广度，最终交汇出判断。

你正在执行一次双轴分析法深度研究。最终产出一份**排版精美的PDF研究报告**。
...
拿到用户输入后，确认以下信息。如果用户已经给得足够明确（比如"帮我用双轴分析法研究Hermes Agent"），不需要追问，直接开始：
...
> - 如果环境中可用 web-access skill，优先加载它并遵循其指引：用 Skill 工具调用 `web-access:web-access`，它提供更强的浏览器CDP能力
> - 搜索策略：先用WebSearch发现信息来源和线索，找到具体URL后用WebFetch深入提取
> - 多次搜索、多个关键词组合，不要只搜一次就放弃
> - 一手来源优于二手来源：官方博客 > 权威媒体原创报道 > 转载/聚合
> - **学术/技术类研究对象必查arxiv**：涉及AI、算法、技术范式时使用。调用方式：`curl -s "https://export.arxiv.org/api/query?search_query=all:关键词1+AND+all:关键词2&max_results=10"`，或用WebFetch访问同一URL。返回XML格式，包含标题、作者、摘要、发布日期、PDF链接。可按需调整关键词组合和结果数量。找到关键论文后，用WebFetch读取论文页面（`https://arxiv.org/abs/论文ID`）获取更多细节。
- **医学/生命科学类研究对象必查PubMed**：通过 PubMed E-utilities API 检索文献。先搜：`curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmax=20&term=关键词"` 获取 PMID 列表，再拉摘要：`curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=PMID1,PMID2,...&rettype=abstract&retmode=text"`。也可用 WebFetch 直接访问 `https://pubmed.ncbi.nlm.nih.gov/?term=关键词` 进行交互式搜索。系统综述/荟萃分析优先检索 Cochrane Library（cochranelibrary.com），临床试验数据检索 ClinicalTrials.gov 或 WHO ICTRP。预印本查 medRxiv（medrxiv.org）和 bioRxiv（biorxiv.org）。
...
| 医学/生命科学 | PubMed / PubMed Central、Cochrane Library（系统综述）、ClinicalTrials.gov / WHO ICTRP（临床试验）、medRxiv / bioRxiv（预印本） |
...
这不是一份冷冰冰的咨询报告，而是一篇让人能从头读到尾的深度研究。写作风格需要在「研究报告的严谨」和「深度叙事的可读性」之间找到平衡点。
...
1. **先完成Markdown稿件**：将完整报告写为标准Markdown格式，保存为 `[研究对象]_双轴分析报告.md`
2. **安装依赖**（如未安装）：`pip install weasyprint markdown --break-system-packages`
3. **运行转换脚本**：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "Jehomn Bea"
   ```
...
- **封面页**：自动生成，包含标题（28pt深蓝色）、副标题「双轴分析法深度研究报告」、作者信息、装饰分隔线
- **配色**：H1标题=#0a1e3c海军蓝、H2=#2c5f7c钢蓝、H3=#4b7d9e中蓝、H4=#5e8ba8灰蓝，正文=#2c3e50深灰，强调金色装饰线=#c8963a
- **字体**：CSS fallback链 `"Droid Sans Fallback", Helvetica, Arial, sans-serif`，自动处理中英文混排
- **正文**：10.5pt，行距1.75，两端对齐，孤行/寡行控制
- **引用块**：左侧3pt钢蓝竖线 + 浅蓝灰背景
- **表格**：全宽、海军蓝表头白字、斑马纹行
- **页眉**：「报告标题 | 双轴分析法深度研究报告」（首页不显示）
- **页脚**：「第 X 页」（首页不显示）
...
PDF文件命名为 `[研究对象名称]_双轴分析报告.pdf`，保存到用户的工作目录中。
