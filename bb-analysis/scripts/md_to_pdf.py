#!/usr/bin/env python3
"""
双轴分析法报告 Markdown → PDF 转换脚本 (WeasyPrint版)
...
h1 {
    ...
    margin-bottom: 6mm;
    padding-bottom: 3mm;
    border-bottom: 2pt solid #0a1e3c;
...
def md_to_html(md_text, title="双轴分析报告", subtitle="双轴分析法深度研究报告",
               meta_line="", author="Jehomn Bea"):
    ...
    if first_h1_match:
        ...
        if not title or title == "双轴分析报告":
            title = extracted_title
        html_body = html_body.replace(first_h1_match.group(0), '', 1)

    # 替换 CSS 中的页眉占位符
    css = CSS_TEMPLATE.replace("HEADER_TEXT", f"{title}  |  双轴分析法深度研究报告")
...
def main():
    parser = argparse.ArgumentParser(description="双轴分析法报告 Markdown → PDF")
    parser.add_argument("input", help="输入的 Markdown 文件路径")
    parser.add_argument("output", help="输出的 PDF 文件路径")
    parser.add_argument("--title", default=None, help="报告标题")
    parser.add_argument("--author", default="Jehomn Bea", help="作者名")
    ...
    html = md_to_html(md_text, title=args.title or "双轴分析报告", meta_line=meta_line, author=args.author)
